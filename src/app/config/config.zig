...
pub const ConfigManager = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Self {
        ...
        std.log.scoped(.config).debug("[ConfigManager.init] Determining executable directory", .{});
        var dir_buf: [std.fs.max_path_bytes]u8 = undefined;
        const exe_dir = try std.fs.selfExeDirPath(&dir_buf);
        std.log.scoped(.config).debug("[ConfigManager.init] Executable directory: {s}", .{exe_dir});

        // 2. Log current working directory for debugging
        var cwd_buf: [std.fs.max_path_bytes]u8 = undefined;
        // Get the current working directory
        const cwd = try std.fs.cwd().realpath(".", &cwd_buf);
        std.log.scoped(.config).debug("[ConfigManager.init] Current working directory: {s}", .{cwd});

        // 3. Define possible config locations
        std.log.scoped(.config).debug("[ConfigManager.init] Determining config file path based on build mode: {s}", .{@tagName(builtin.mode)});
        ...
        std.log.scoped(.config).debug("[ConfigManager.init] Searching for config file in {d} possible locations", .{config_paths.len});
        for (config_paths) |path_info| {
            if (path_info.base.len == 0) {
                std.log.scoped(.config).debug("[ConfigManager.init] Skipping empty base path", .{});
            ...
            std.log.scoped(.config).debug("[ConfigManager.init] Trying config path: {s}", .{full_path});

            // Check if file exists
            const file = std.fs.openFileAbsolute(full_path, .{ .mode = .read_only }) catch |err| {
                std.log.scoped(.config).debug("[ConfigManager.init] Could not open {s}: {s}", .{ full_path, @errorName(err) });
                continue;
            };
            file.close();
        ...
        if (config_file_path == null) {
            std.log.scoped(.config).err("[ConfigManager.init] Could not find config file in any location", .{});
        ...
        std.log.scoped(.config).debug("[ConfigManager.init] Loading configuration from file", .{});
        try manager.load();
        std.log.scoped(.config).debug("[ConfigManager.init] Configuration manager initialized successfully", .{});

        return manager;
    }

    pub fn deinit(self: *Self) void {
        std.log.scoped(.config).debug("[ConfigManager.deinit] Cleaning up configuration manager resources", .{});
        ...
        std.log.scoped(.config).debug("[ConfigManager.deinit] Configuration manager resources freed", .{});
    }

    pub fn createDefaultConfigFile(self: *Self, path: []const u8) !void {
        std.log.scoped(.config).info("[ConfigManager.createDefaultConfigFile] Creating default config file at: {s}", .{path});
        ...
        std.log.scoped(.config).info("[ConfigManager.createDefaultConfigFile] Default configuration saved to {s}", .{path});
    }
    ...
    fn parseAuthStrategy(str: []const u8) !types.AuthStrategy {
        std.log.scoped(.config).debug("[ConfigManager.parseAuthStrategy] Parsing auth strategy: '{s}'", .{str});
        ...
        std.log.scoped(.config).err("[ConfigManager.parseAuthStrategy] Invalid auth strategy: '{s}'", .{str});
    ...
    fn parseSecurityEvent(str: []const u8) !types.SecurityEvent {
        std.log.scoped(.config).debug("[ConfigManager.parseSecurityEvent] Parsing security event: '{s}'", .{str});
        ...
        std.log.scoped(.config).err("[ConfigManager.parseSecurityEvent] Invalid security event: '{s}'", .{str});
        return error.InvalidSecurityEvent;
    }

    fn parseOAuthProvider(str: []const u8) !types.OAuthProvider {
        std.log.scoped(.config).debug("[ConfigManager.parseOAuthProvider] Parsing OAuth provider: '{s}'", .{str});
        ...
        std.log.scoped(.config).err("[ConfigManager.parseOAuthProvider] Invalid OAuth provider: '{s}'", .{str});
        return error.InvalidOAuthProvider;
    }

    pub fn load(self: *Self) !void {
        std.log.scoped(.config).debug("[ConfigManager.load] Starting load from config file: {s}", .{self.config_file_path});

        const file = std.fs.openFileAbsolute(self.config_file_path, .{ .mode = .read_only }) catch |err| {
            std.log.scoped(.config).err("[ConfigManager.load] Error opening config file: {s}", .{@errorName(err)});
            return err;
        };
        defer file.close();

        const file_size = try file.getEndPos();
        std.log.scoped(.config).debug("[ConfigManager.load] Config file size: {d} bytes", .{file_size});

        const file_content = try self.allocator.alloc(u8, file_size);
        defer self.allocator.free(file_content);

        const bytes_read = try file.readAll(file_content);
        std.log.scoped(.config).debug("[ConfigManager.load] Read {d} bytes from config file", .{bytes_read});
        std.log.scoped(.config).debug("[ConfigManager.load] Config file content: '{s}'", .{file_content});

        // Parse the JSON
        std.log.scoped(.config).debug("[ConfigManager.load] Parsing JSON content", .{});
        var parsed_json = try std.json.parseFromSlice(std.json.Value, self.allocator, file_content, .{});
        defer parsed_json.deinit();

        // Get the Redis config
        std.log.scoped(.config).debug("[ConfigManager.load] Processing Redis configuration", .{});
        if (parsed_json.value.object.get("redis")) |redis_json| {
            ...
            std.log.scoped(.config).debug("[ConfigManager.load] Redis config: host={s}, port={d}, max_connections={d}", .{
                self.redis_config.host,
                self.redis_config.port,
                self.redis_config.max_connections,
            });
        } else {
            std.log.scoped(.config).debug("[ConfigManager.load] No Redis configuration found, using defaults", .{});
        }

        // Get the security config
        std.log.scoped(.config).debug("[ConfigManager.load] Processing security configuration", .{});
        if (parsed_json.value.object.get("security")) |security_json| {
            // Parse middleware
            std.log.scoped(.config).debug("[ConfigManager.load] Processing auth middleware configuration", .{});
            if (security_json.object.get("middleware")) |middleware_json| {
                ...
                if (middleware_json.object.get("protected_routes")) |routes_json| {
                    std.log.scoped(.config).debug("[ConfigManager.load] Processing {d} protected routes", .{routes_json.array.items.len});
                    for (routes_json.array.items, 0..) |route_json, i| {
                        ...
                        std.log.scoped(.config).debug("[ConfigManager.load] Processing protected route {d}: prefix={s}", .{ i, prefix });

                        var required_roles: ?[]const []const u8 = null;
                        if (route_json.object.get("required_roles")) |roles_json| {
                            if (roles_json != .null) {
                                std.log.scoped(.config).debug("[ConfigManager.load] Processing {d} required roles for route", .{roles_json.array.items.len});
                                ...
                                std.log.scoped(.config).debug("[ConfigManager.load] No required roles for route", .{});
                            }
                        }
                ...
                std.log.scoped(.config).debug("[ConfigManager.load] Processed {d} protected routes total", .{self.security_config.middleware.protected_routes.len});
                ...
                if (middleware_json.object.get("login_redirect_url")) |url_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set login redirect URL: {s}", .{self.security_config.middleware.login_redirect_url});
                }
                ...
                if (middleware_json.object.get("use_return_to")) |use_return_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set use_return_to: {}", .{self.security_config.middleware.use_return_to});
                }
                ...
                if (middleware_json.object.get("api_error_message")) |msg_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set API error message: {s}", .{self.security_config.middleware.api_error_message});
                }
            }

            std.log.scoped(.config).debug("[ConfigManager.load] Processing session configuration", .{});
            if (security_json.object.get("session")) |session_json| {
                ...
                if (session_json.object.get("session_ttl")) |session_ttl_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set session TTL: {d} seconds", .{self.security_config.session.session_ttl});
                }
                ...
                if (session_json.object.get("refresh_threshold")) |refresh_threshold_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set refresh threshold: {d} seconds", .{self.security_config.session.refresh_threshold});
                }
                ...
                if (session_json.object.get("cleanup_interval")) |cleanup_interval_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set cleanup interval: {d} seconds", .{self.security_config.session.cleanup_interval});
                }
            }

            // Parse storage config
            std.log.scoped(.config).debug("[ConfigManager.load] Processing storage configuration", .{});
            if (security_json.object.get("storage")) |storage_json| {
                if (storage_json.object.get("storage_type")) |storage_type_json| {
                    std.log.scoped(.config).debug("[ConfigManager.load] Processing storage type: {s}", .{storage_type_json.string});
                    if (std.mem.eql(u8, storage_type_json.string, "redis")) {
                        ...
                        std.log.scoped(.config).warn("[ConfigManager.load] Unknown storage type: {s}, defaulting to 'both'", .{storage_type_json.string});
                        self.security_config.storage.storage_type = .both;
                    }
                }
                ...
                if (storage_json.object.get("cleanup_batch_size")) |cleanup_batch_size_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set cleanup batch size: {d}", .{self.security_config.storage.cleanup_batch_size});
                }
            }

            // Parse tokens config
            std.log.scoped(.config).debug("[ConfigManager.load] Processing token configuration", .{});
            if (security_json.object.get("token")) |token_json| {
                if (token_json.object.get("access_token_ttl")) |access_token_ttl_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set access token TTL: {d} seconds", .{self.security_config.token.access_token_ttl});
                }
                ...
                if (token_json.object.get("refresh_token_ttl")) |refresh_token_ttl_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set refresh token TTL: {d} seconds", .{self.security_config.token.refresh_token_ttl});
                }
                ...
                if (token_json.object.get("token_length")) |token_length_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set token length: {d}", .{self.security_config.token.token_length});
                }
            }

            // Parse rate limit config
            std.log.scoped(.config).debug("[ConfigManager.load] Processing rate limit configuration", .{});
            if (security_json.object.get("rate_limit")) |rate_limit_json| {
                if (rate_limit_json.object.get("max_attempts")) |max_attempts_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set max attempts: {d}", .{self.security_config.rate_limit.max_attempts});
                }
                ...
                if (rate_limit_json.object.get("window_seconds")) |window_seconds_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set window seconds: {d}", .{self.security_config.rate_limit.window_seconds});
                }
                ...
                if (rate_limit_json.object.get("lockout_duration")) |lockout_duration_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set lockout duration: {d} seconds", .{self.security_config.rate_limit.lockout_duration});
                }
            }

            // Parse audit config
            std.log.scoped(.config).debug("[ConfigManager.load] Processing audit configuration", .{});
            if (security_json.object.get("audit")) |audit_json| {
                if (audit_json.object.get("enabled")) |enabled_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set audit enabled: {}", .{self.security_config.audit.enabled});
                }
                ...
                if (audit_json.object.get("notify_admins")) |notify_admins_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set notify admins: {}", .{self.security_config.audit.notify_admins});
                }

                if (audit_json.object.get("store_type")) |store_type_json| {
                    std.log.scoped(.config).debug("[ConfigManager.load] Processing audit store type: {s}", .{store_type_json.string});
                    if (std.mem.eql(u8, store_type_json.string, "redis")) {
                        ...
                        std.log.scoped(.config).warn("[ConfigManager.load] Unknown audit store type: {s}, defaulting to 'both'", .{store_type_json.string});
                        self.security_config.audit.store_type = .both;
                    }
                }
                ...
                if (audit_json.object.get("log_retention_days")) |log_retention_days_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set log retention days: {d}", .{self.security_config.audit.log_retention_days});
                }

                // Parse high_risk_events array
                if (audit_json.object.get("high_risk_events")) |high_risk_events_json| {
                    std.log.scoped(.config).debug("[ConfigManager.load] Processing {d} high risk events", .{high_risk_events_json.array.items.len});
                    ...
                    for (high_risk_events_json.array.items, 0..) |event_json, i| {
                        ...
                        std.log.scoped(.config).debug("[ConfigManager.load] Processing high risk event {d}: {s}", .{ i, event_str });
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set {d} high risk events", .{self.security_config.audit.high_risk_events.len});
                }
            }

            std.log.scoped(.config).debug("[ConfigManager.load] Processing OAuth configuration", .{});
            if (security_json.object.get("oauth")) |oauth_json| {
                ...
                if (oauth_json.object.get("enabled")) |enabled_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set OAuth enabled: {}", .{self.security_config.oauth.enabled});
                }
                ...
                if (oauth_json.object.get("state_cookie_name")) |state_cookie_name_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set OAuth state_cookie_name: {s}", .{self.security_config.oauth.state_cookie_name});
                }
                ...
                if (oauth_json.object.get("state_cookie_max_age")) |state_cookie_max_age_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set OAuth state_cookie_max_age: {d} seconds", .{self.security_config.oauth.state_cookie_max_age});
                }
                ...
                if (oauth_json.object.get("default_redirect")) |redirect_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set OAuth default redirect: {s}", .{self.security_config.oauth.default_redirect});
                }
                ...
                if (oauth_json.object.get("user_auto_create")) |auto_create_json| {
                    ...
                    std.log.scoped(.config).debug("[ConfigManager.load] Set OAuth user auto create: {}", .{self.security_config.oauth.user_auto_create});
                }

                if (oauth_json.object.get("user_auto_login")) |auto_login_json| {
                    self.security_config.oauth.user_auto_login = auto_login_json.bool;
                    std.log.scoped(.config).debug("[ConfigManager.load] Set OAuth user auto login: {}", .{self.security_config.oauth.user_auto_login});
                }

                // Parse OAuth providers
                if (oauth_json.object.get("providers")) |providers_json| {
                    std.log.scoped(.config).debug("[ConfigManager.load] Processing {d} OAuth providers", .{providers_json.array.items.len});
                    ...
                    for (providers_json.array.items, 0..) |provider_json, i| {
                        std.log.scoped(.config).debug("[ConfigManager.load] Processing OAuth provider {d}", .{i});
                        ...
                        std.log.scoped(.config).debug("[ConfigManager.load] Added OAuth provider: {s}", .{name});
                    }

                    self.security_config.oauth.providers = try providers.toOwnedSlice();
                    std.log.scoped(.config).debug("[ConfigManager.load] Processed {d} OAuth providers total", .{self.security_config.oauth.providers.len});
                }
            }
        } else {
            std.log.scoped(.config).debug("[ConfigManager.load] No security configuration found, using defaults", .{});
        }

        std.log.scoped(.config).info("[ConfigManager.load] Configuration loaded successfully from {s}", .{self.config_file_path});
    }

    pub fn save(self: *Self) !void {
        std.log.scoped(.config).debug("[ConfigManager.save] Starting save to config file: {s}", .{self.config_file_path});
        ...
        std.log.scoped(.config).debug("[ConfigManager.save] Serializing configuration to JSON", .{});
        try json_writer.writeStruct(app_config);

        const json_content = json_writer.getWritten();
        std.log.scoped(.config).debug("[ConfigManager.save] Writing {d} bytes to config file", .{json_content.len});
        _ = try file.writeAll(json_content);

        std.log.scoped(.config).info("[ConfigManager.save] Configuration saved successfully to {s}", .{self.config_file_path});
    }
};
