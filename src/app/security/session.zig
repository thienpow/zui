...
pub const SessionManager = struct {
    ...
    pub fn create(self: *SessionManager, user: User, request: *jetzig.Request) !Session {
        std.log.scoped(.auth).debug("[session.create] Checking active session count for user_id: {}", .{user.id});
        ...
        if (active_sessions.len >= self.config.max_sessions_per_user) {
            if (active_sessions.len > 0) {
                std.log.scoped(.auth).debug("[session.create] Max sessions ({d}) reached", .{
                    self.config.max_sessions_per_user,
                });
                try self.storage.invalidateSession(active_sessions[0].id);
            } else {
                std.log.scoped(.auth).debug("[session.create] Max sessions ({d}) reached, but no existing sessions found to invalidate", .{
                    self.config.max_sessions_per_user,
                });
            }
        }

        std.log.scoped(.auth).debug("[session.create] Creating new session for user_id: {}", .{user.id});
        ...
        std.log.scoped(.auth).debug("[session.create] Saving session with id: '{s}', token: '{s}'", .{ session.id, session.token });
        try self.storage.saveSession(session);

        std.log.scoped(.auth).debug("[session.create] Setting session cookie with token: '{s}'", .{session.token});
        try self.setSessionCookie(request, session.token);

        std.log.scoped(.auth).debug("[session.create] Session created successfully for user_id: {}", .{user.id});
        return session;
    }

    pub fn validate(self: *SessionManager, token: []const u8, request: *jetzig.Request) !Session {
        std.log.scoped(.auth).debug("[session.validate] Token received: '{s}' (length: {})", .{ token, token.len });

        const session = try self.storage.getSessionByToken(token) orelse {
            std.log.scoped(.auth).debug("[session.validate] No session found for token: '{s}'", .{token});
            try self.clearSessionCookie(request);
            return error.InvalidSession;
        };

        std.log.scoped(.auth).debug("[session.validate] Session found: id='{s}', user_id={d}, token='{s}'", .{ session.id, session.user_id, session.token });
        std.log.scoped(.auth).debug("[session.validate] Session metadata: ip_address='{s}', user_agent='{s}'", .{ session.metadata.ip_address orelse "null", session.metadata.user_agent orelse "null" });

        if (self.isExpired(session)) {
            std.log.scoped(.auth).debug("[session.validate] Session expired for token: '{s}', id: '{s}'", .{ token, session.id });
        ...
        if (self.needsRefresh(session)) {
            std.log.scoped(.auth).debug("[session.validate] Session needs refresh for token: '{s}', id: '{s}'", .{ token, session.id });
            return try self.refresh(session, request);
        }

        std.log.scoped(.auth).debug("[session.validate] Session valid for token: '{s}', id: '{s}'", .{ token, session.id });
        return session;
    }
    ...
    pub fn setSessionCookie(self: *SessionManager, request: *jetzig.Request, token: []const u8) !void {
        std.log.scoped(.auth).debug("[session.setSessionCookie] Starting cookie set for token: '{s}'", .{token});
        const cookies = try request.cookies();
        std.log.scoped(.auth).debug("[session.setSessionCookie] Cookies object retrieved", .{});
        ...
        std.log.scoped(.auth).debug("[session.setSessionCookie] Cookie 'session_token' set with value: '{s}', max_age: {}", .{ token, self.config.session_ttl });
    }
    ...
    pub fn getSessionTokenFromCookie(self: *SessionManager, request: *jetzig.Request) !?[]const u8 {
        std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] Starting cookie retrieval", .{});

        // Attempt to get the cookies object from the request
        std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] Calling request.cookies()", .{});
        const cookies = try request.cookies();
        std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] Successfully retrieved cookies object", .{});

        // Check if the session_token cookie exists
        std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] Looking for 'session_token' cookie", .{});
        if (cookies.get(self.config.cookie_name)) |cookie| {
            std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] Found 'session_token' cookie with value: '{s}'", .{cookie.value});
            std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] Returning token: '{s}' (length: {})", .{ cookie.value, cookie.value.len });
            return cookie.value;
        } else {
            std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] No 'session_token' cookie found", .{});
            std.log.scoped(.auth).debug("[session.getSessionTokenFromCookie] Returning null", .{});
            return null;
        }
    }
