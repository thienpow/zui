...
pub const SessionStorage = struct {
    ...
    pub fn getSessionByToken(self: *SessionStorage, token: []const u8) !?Session {
        std.log.info("[storage.getSessionByToken] token={s}", .{token});
        ...
        const data = (try client.get(key)) orelse {
            std.log.info("[storage.getSessionByToken] Key not found: {s}", .{key});
            return null;
        };

        std.log.info("[storage.getSessionByToken] Raw data retrieved: {s}", .{data});
        ...
        std.log.info("[storage.getSessionByToken] Parsed session ID: {s}", .{session.id});
        return session;
    }

    pub fn getSessionById(self: *SessionStorage, session_id: []const u8) !?Session {
        std.log.info("[storage.getSessionById] session_id={s}", .{session_id});
        var client = try self.redis_pool.acquire();
        defer self.redis_pool.release(client);
        ...
        // Handle the optional result from get()
        const data = (try client.get(key)) orelse {
            std.log.info("[storage.getSessionById] Key not found: {s}", .{key});
            return null;
        };
    ...
    pub fn invalidateSession(self: *SessionStorage, session_id: []const u8) !void {
        ...
        const session = (try self.getSessionById(session_id)) orelse {
            std.log.info("[storage.invalidateSession] Session not found: {s}", .{session_id});
            return;
        };
    ...
    pub fn saveSession(self: *SessionStorage, session: Session) !void {
        ...
        std.log.debug("[storage.saveSession] Saving session: id='{s}' (len: {})", .{ session.id, session.id.len });
        std.log.debug("[storage.saveSession] User ID: {d}, token='{s}' (len: {})", .{ session.user_id, session.token, session.token.len });
        ...
        std.log.debug("[storage.saveSession] Metadata: ip_address={s}, user_agent={s}", .{ ip_str, ua_str });
        ...
        std.log.debug("[storage.saveSession] JSON serialized: {s}", .{value});
