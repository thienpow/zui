...
pub fn set_cookie(request: *jetzig.Request, name: []const u8, value: []const u8) !void {
    ...
    cookies.put(jetzig.http.Cookies.Cookie{
        .name = name,
        .value = value,
        ...
        std.log.scoped(.utils).err("[cookie.set_cookie] Failed to set cookie: {any}", .{err});
    };
}
...
pub fn set_oauth_cookie(request: *jetzig.Request, value: []const u8) !void {
    ...
    cookies.put(jetzig.http.Cookies.Cookie{
        ...
        std.log.scoped(.utils).err("[cookie.set_oauth_cookie] Failed to set cookie: {any}", .{err});
    };
}
